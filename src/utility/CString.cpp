...
{
	return ( compare == CaseSensitive ? strcmp : stricmp )( pszString1, pszString2 );
}

int CompareN( const char* pszString1, const char* pszString2, const size_t uiCount, const CompareType compare )
{
	return ( compare == CaseSensitive ? strncmp : strnicmp )( pszString1, pszString2, uiCount );
}
...
{
	return strnicmp( CStr(), pszString, uiCount );
}
...
CString::size_type CString::FindLastNotOf( const CString& str, size_t uiStartIndex, const String::CompareType compare ) const
{
	...
	while (uiStartIndex != INVALID_INDEX && str.FindLastOf( *( m_pszString + uiStartIndex ), 0, compare ) != INVALID_INDEX )
		--uiStartIndex;
